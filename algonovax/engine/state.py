...
def load_state(path: str, default: EngineState) -> EngineState:
    ...
    try:
        ...
        default.unrealized_pnl = float(
            raw.get("unrealized_pnl", default.unrealized_pnl)
        )
        default.equity = float(raw.get("equity", default.equity))
        default.last_price = float(raw.get("last_price", default.last_price))
        default.day_start_equity = float(
            raw.get("day_start_equity", default.day_start_equity)
        )

        default.position.side = str(pos.get("side", default.position.side))
        default.position.qty_base = float(
            pos.get("qty_base", default.position.qty_base)
        )
        default.position.entry_price = float(
            pos.get("entry_price", default.position.entry_price)
        )
...
def save_state(path: str, st: EngineState) -> None:
    ...
    p.write_text(
        json.dumps(st.to_dict(), separators=(",", ":"), ensure_ascii=False),
        encoding="utf-8",
    )
