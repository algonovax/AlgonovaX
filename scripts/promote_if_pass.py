...
def main() -> int:
    ...
    max_dd_pct = float(os.environ.get("PROMOTE_MAX_DD_PCT", "1.0"))  # only used if dd key exists

    ok_rows = []
    for r in rows:
        ok = bool(r.get("ok"))
        trades = int(r.get("trades") or (r.get("report") or {}).get("trades") or 0)
        pnl = _f(r.get("pnl") if "pnl" in r else (r.get("report") or {}).get("pnl"), -1e18)
    ...
    if not ok_rows:
        ...
        rows_sorted = sorted(rows, key=lambda r: _f(r.get("pnl") if "pnl" in r else (r.get("report") or {}).get("pnl"), -1e18), reverse=True)
        print("No candidates passed gates.")
        print(f"GATES: ok=true, trades>={min_trades}, pnl>{min_pnl}, dd_pct<={max_dd_pct} (only if dd metric present)")
        print("Top 5 by pnl:")
        for r in rows_sorted[:5]:
            ...
            print(f"- pnl={pnl:.6f} trades={trades} ok={bool(r.get('ok'))} dd_pct={dd_pct} params={r.get('params')}")
        return 0

    best = sorted(ok_rows, key=lambda r: _f(r.get("pnl") if "pnl" in r else (r.get("report") or {}).get("pnl"), -1e18), reverse=True)[0]

    OUT_PATH.parent.mkdir(parents=True, exist_ok=True)
    with OUT_PATH.open("a", encoding="utf-8") as f:
        f.write(json.dumps(best, ensure_ascii=False) + "\n")

    pnl = _f(best.get("pnl") if "pnl" in best else (best.get("report") or {}).get("pnl"), 0.0)
